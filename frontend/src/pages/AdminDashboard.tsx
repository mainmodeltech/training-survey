...
export default function AdminDashboard({ token, user, onLogout }: Props) {
  ...
  const loadSubmissions = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchSubmissions(token, params as { level?: string; reviewed?: boolean });
  ...
  const openDetail = async (id: number) => {
    const d = await fetchSubmissionDetail(token, id);
    setDetail(d as Record<string, unknown>);
    setSelectedId(id);
    setAdminNote((d as Record<string, unknown>).notes_admin as string || "");
    setTab("detail");
  };
        ...
        {tab === "detail" && detail && (
          <div className="admin-content">
            ...
            <div className="detail-header">
              <div>
                <h1 className="admin-page-title">{detail.full_name as string}</h1>
                <p className="admin-page-sub">{detail.position as string} · {detail.department as string} · {detail.location as string}</p>
              </div>
              <div className="detail-badges">
                <span className="level-pill" style={{ background: levelColor(detail.computed_level as string), fontSize: "14px", padding: "6px 16px" }}>
                  Curriculat {detail.computed_level as string}
                </span>
                <span className="score-badge">Score : {detail.score_total as number}/100</span>
              </div>
            </div>
            ...
            <div className="detail-grid">
              <div className="detail-section">
                ...
                <div className="detail-row"><span>Niveau déclaré</span><strong>{detail.self_level as string}</strong></div>
                <div className="detail-row"><span>Taille fichiers</span><strong>{(detail.file_size as string) || "—"}</strong></div>
                <div className="detail-features">
                  {Object.entries((detail.features_usage as Record<string, string>) || {}).map(([f, u]) => (
              ...
              <div className="detail-section">
                ...
                <div className="detail-row"><span>Temps / jour</span><strong>{detail.daily_time as string}</strong></div>
                <div className="detail-tags">
                  {((detail.daily_tasks as string[]) || []).map((t, i) => <span key={i} className="detail-tag">{t}</span>)}
                </div>
                <h4 className="detail-sub-title">Difficultés</h4>
                <div className="detail-tags">
                  {((detail.difficulties as string[]) || []).map((d, i) => <span key={i} className="detail-tag detail-tag--red">{d}</span>)}
                </div>
                {detail.difficulty_details && (
                  <div className="detail-note">{detail.difficulty_details as string}</div>
                )}
              </div>

              <div className="detail-section">
                <h3 className="detail-section-title">Ambitions</h3>
                <div className="detail-row"><span>Motivation</span><strong>{detail.motivation as string}</strong></div>
                <div className="detail-row"><span>Ambition</span><strong>{detail.ambition_level as string}</strong></div>
                <h4 className="detail-sub-title">Résultats attendus</h4>
                <div className="detail-tags">
                  {((detail.expected_results as string[]) || []).map((r, i) => <span key={i} className="detail-tag detail-tag--green">{r}</span>)}
                </div>
              </div>

              <div className="detail-section">
                <h3 className="detail-section-title">Modalités</h3>
                <div className="detail-row"><span>Durée</span><strong>{detail.preferred_duration as string}</strong></div>
                <div className="detail-row"><span>Format</span><strong>{detail.preferred_modality as string}</strong></div>
                <div className="detail-row"><span>Ordinateur</span><strong>{(detail.has_computer as string) || "—"}</strong></div>
                {detail.concrete_case && (
                  <>
                    <h4 className="detail-sub-title">Cas concret</h4>
                    <div className="detail-note">{detail.concrete_case as string}</div>
                  </>
                )}
                {detail.additional_comments && (
                  <>
                    <h4 className="detail-sub-title">Remarques</h4>
                    <div className="detail-note">{detail.additional_comments as string}</div>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="admin-note-card">
              ...
              <div style={{ display: "flex", gap: "12px", marginTop: "12px" }}>
                ...
                {detail.reviewed && <span className="status-pill status-pill--done" style={{ alignSelf: "center" }}>Traité</span>}
              </div>
            </div>
          </div>
        )}
